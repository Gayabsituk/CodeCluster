...
public class CodeCluster extends JFrame {
    ...
    public void startGame(String playerName) {
        ...
        this.currentLevel = 1;
        levelScores.clear(); // Reset scores for new game
    ...
    public TreeMap<Integer, Integer> getLevelScores() {
        return levelScores;
    }
...
class GameBoardPanel extends JPanel {
    ...
    private void endGame() {
        ...
        if (levelCompleted && totalScore > 0) {
            ...
            // Save level score for summary
            game.setLevelScore(level, totalScore);
        }
        ...
        // Determine if this is a final game over (failed or completed level 5)
        boolean isFinalGameOver = !levelCompleted || level == 5;
        showGameCompleteDialog(totalScore, achievements, levelCompleted, isFinalGameOver);
    }
    ...
    private void showGameCompleteDialog(int score, ArrayList<String> achievements, boolean levelCompleted, boolean isFinalGameOver) {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Code Cluster", true);
        int dialogHeight = levelCompleted ? 650 : 520;
        // Add extra height for score summary if it's a final game over
        if (isFinalGameOver && !game.getLevelScores().isEmpty()) {
            dialogHeight += 150;
        }
        dialog.setSize(500, dialogHeight);
        ...
        // Add score summary if it's a final game over
        if (isFinalGameOver && !game.getLevelScores().isEmpty()) {
            DarkRoundedPanel scoreSummaryPanel = new DarkRoundedPanel();
            scoreSummaryPanel.setLayout(new BoxLayout(scoreSummaryPanel, BoxLayout.Y_AXIS));
            scoreSummaryPanel.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));
            scoreSummaryPanel.setMaximumSize(new Dimension(420, 140));
            
            JLabel summaryTitle = new JLabel("Game Over - Level Summary:", SwingConstants.CENTER);
            ...
            scoreSummaryPanel.add(summaryTitle);
            scoreSummaryPanel.add(Box.createRigidArea(new Dimension(0, 10)));
            
            TreeMap<Integer, Integer> levelScores = game.getLevelScores();
            for (Map.Entry<Integer, Integer> entry : levelScores.entrySet()) {
                JLabel levelScoreLabel = new JLabel("Lvl " + entry.getKey() + ": " + entry.getValue(), SwingConstants.CENTER);
                levelScoreLabel.setFont(new Font("Segoe UI", Font.PLAIN, 16));
                levelScoreLabel.setForeground(new Color(156, 163, 175));
                levelScoreLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
                scoreSummaryPanel.add(levelScoreLabel);
            }
            
            mainPanel.add(scoreSummaryPanel);
        }
